...
def execute_sparse_views(
    ...
    *,
    ...
    all_chunks: list[tuple[str, RequestChunk]] = []
    ...
    for chunk_idx, (view, chunk) in enumerate(all_chunks, start=1):
        view_label = view_labels.get(view, view)
        candidate_count = len(chunk.candidates)
        if progress_callback:
            progress_callback(
                (chunk_idx - 1) / max(1, total_chunks),
                f"Flash-Lite 2차 군집 [{view_label} {chunk_idx}/{total_chunks}] 요청 중 ({candidate_count:,}개)",
            )
        ...
        if progress_callback:
            progress_callback(
                chunk_idx / max(1, total_chunks),
                f"Flash-Lite 2차 군집 [{view_label} {chunk_idx}/{total_chunks}] 완료",
            )
